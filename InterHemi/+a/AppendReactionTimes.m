function SL = AppendReactionTimes(SL, plotit)
%
...
%     end
    ...
    threshold = 1/3; %third of max
    
    inds = -window*SL(i).fs:1:0; % indeces to look backward
    ...
    ts = linspace(-window*accelfs,0, length(inds));
...
%     for k = 8;%1:length(bad2)%1:size(SL(i).righttrials,1)
%         I =k;% bad2(k);
% %         if(~isnan(SL(i).rts_r(I)))
%             start = SL(i).righttrials(I,2)*SL(i).fs/1000;
%             plot(RFilter(round(start-window):round(start)));
%             hold on;
% %             rt = SL(i).rts_r(I)*SL(i).fs/1000;
% %             plot([rt,rt],ylim);
% %         end
%     end
end
