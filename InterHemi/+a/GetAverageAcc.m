function [accs,ts] = GetAverageAcc(SL,window)
...
col = 1;
for tstart = {SL.lefttrials(left,1), SL.righttrials(right,1)}; % for left and right targets
    
    windowsamples = round(0:window(2)*SL.fs);
    inds = repmat((tstart{1}*SL.fs/1000+(window(1)*SL.fs)), 1, length(windowsamples)) + repmat(windowsamples, length(tstart{1}), 1);
    inds = round(inds);
