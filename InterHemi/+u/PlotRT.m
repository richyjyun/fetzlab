...
function [c,p,g,l,c2] = PlotRT(SL, Split, c2,g)
%
...
c = figure('position', [0 0 900 1100], 'paperposition', [-.8 -.6 10 8]);
p = figure('position', [0 0 900 1100], 'paperposition', [-.8 -.6 10 8]);
if ~exist('g')
    g = figure('position', [0 0 900 1100], 'paperposition', [-.8 -.6 10 8]);
end
l = figure('position', [0 0 900 1100], 'paperposition', [-.8 -.6 10 8]);

if ~exist('c2', 'var')
    c2 = figure('position', [0 0 900 1100], 'paperposition', [-.8 -.6 10 12]);
end
...
%         text(SL(i).NormDelay+10,L(3)-L(1),char(D),'FontSize',7);
    ...
    elseif(cond == 1 & ~(Split & stimprob>.8))
        ...
        scatter(SL(i).NormDelay,R(2)-R(1),'k');
        err = sqrt( nanstd(ContRT(ind_Cont == 0))^2 / sum(~isnan(ContRT(ind_Cont == 0))) + nanstd(ContRT(ind_Cont == 1))^2 / sum(~isnan(ContRT(ind_Cont == 1))) );
        errorbar(SL(i).NormDelay,R(2)-R(1),err,'k');
        errorbar(SL(i).NormDelay,R(2)-R(1),SL(i).NormSE,'horizontal','k'); hold off;
%         text(SL(i).NormDelay+10,R(2)-R(1),char(D),'FontSize',7);
        ...
        if ~Split
            ...
            err = sqrt( nanstd(IpsiRT(ind_Ipsi == 0))^2/sum(~isnan(IpsiRT(ind_Ipsi == 0))) + nanstd(IpsiRT(ind_Ipsi == 1 & ~stimTrials))^2 / sum(~isnan(IpsiRT(ind_Ipsi == 1 & ~stimTrials))) );
            err2 = sqrt( nanstd(IpsiRT(ind_Ipsi == 0))^2/sum(~isnan(IpsiRT(ind_Ipsi == 0))) + nanstd(IpsiRT(ind_Ipsi == 1 & stimTrials))^2 / sum(~isnan(IpsiRT(ind_Ipsi == 1 & stimTrials))) );
        end
...
%         text(SL(i).NormDelay+10,L(2)-L(1),char(D),'FontSize',7);
        if(strcmp(SL(i).Animal,'Ubi') && Split)
            hold on;
            scatter(SL(i).NormDelay,LStim-L(1),'r')
            errorbar(SL(i).NormDelay,LStim-L(1),err2,'r');
%             text(SL(i).NormDelay+10,LStim-L(1),char(D),'FontSize',7,'Color','r');
        end
