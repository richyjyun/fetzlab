...
% load('F:\Dropbox\repos\abogaard\efetz\U\Code\+u\MetaDataIgorTime.mat');
...
for n = 1:length(SLNeuroTime)
    ...
    if(strcmp(SL(i).StimHemi,'L'))
        temp = SLNeuroTime(n).Rgamma(:,Rchn); temp = cellfun(@transpose,temp,'Uniformoutput',false);
        ContRT.C = cellfun(@(col) vertcat(col{:}), num2cell(temp, 1), 'UniformOutput', false);
        temp = SLNeuroTime(n).Rgamma(:,Lchn); temp = cellfun(@transpose,temp,'Uniformoutput',false);
        ContRT.I = cellfun(@(col) vertcat(col{:}), num2cell(temp, 1), 'UniformOutput', false);
        temp = SLNeuroTime(n).Lgamma(:,Rchn); temp = cellfun(@transpose,temp,'Uniformoutput',false);
        IpsiRT.C = cellfun(@(col) vertcat(col{:}), num2cell(temp, 1), 'UniformOutput', false);
        temp = SLNeuroTime(n).Lgamma(:,Lchn); temp = cellfun(@transpose,temp,'Uniformoutput',false);
    ...
    for t = start:finish
        ...
        if(Labels(t))
            Cind(end+1) = sum(Labels(1:t)); %find nth contra trial
            stimind = sum(Stim(1:t-since));
            StimC(end+1) = stimT(stimind);
        else
            Iind(end+1) = sum(~Labels(1:t)); %find nth ipsi trial
            stimind = sum(Stim(1:t-since));
    ...
    Cont.C{end+1} = cellfun(@(x,y) x(Cind,:)-y,ContRT.C,CPre.C,'UniformOutput',false);
    Cont.I{end+1} = cellfun(@(x,y) x(Cind,:)-y,ContRT.I,CPre.I,'UniformOutput',false);
    Ipsi.C{end+1} = cellfun(@(x,y) x(Iind,:)-y,IpsiRT.C,IPre.C,'UniformOutput',false);
    Ipsi.I{end+1} = cellfun(@(x,y) x(Iind,:)-y,IpsiRT.I,IPre.I,'UniformOutput',false);
...
for d = 1:length(Cont.C)
    ...
    for b = 1:size(bins,1)
        ...
        for t = 1:2 % type of trial
            ...
            for i = 1:3 % 3 points that were measured
                
                stimT = StimAll.(side){d};
                ind = stimT >= bins(b,1) & stimT < bins(b,2);
                temp = cellfun(@(x) x(ind,i),Trial.C{d},'UniformOutput',false);
                temp = [temp{:}]; temp = temp(:);
                Beta.C = [Beta.C;temp];
                Group.C = [Group.C;bcounter*ones(length(temp),1)];
                
                temp = cellfun(@(x) x(ind,i),Trial.I{d},'UniformOutput',false);
                temp = [temp{:}]; temp = temp(:);
                Beta.I = [Beta.I;temp];
                Group.I = [Group.I;bcounter*ones(length(temp),1)];
                bcounter = bcounter+1;
            end
        end
    end
...
title('Contra Exp, Gamma, Contra Hemi'); 
hold on; xl = xlim;
plot(xl,[0,0],'k--')
...
title('Contra Exp, Gamma, Ipsi Hemi'); 
hold on; xl = xlim;
plot(xl,[0,0],'k--')
boxplot(Beta.I,Group.I,'Notch','on', 'positions', positions,'Whisker',0,'Symbol','');
ylim([-3,3])
