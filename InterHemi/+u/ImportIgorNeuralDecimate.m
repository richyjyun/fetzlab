...
if ~exist('fs_target', 'var'), fs_target = 333; end
...
fseek(fid,0,'bof');
data2 = fread(fid,[1,inf],precision); %[9, inf], 'single')'; % note the transpose to put data in columns
data2 = reshape(data2,[Nch,length(data2)/Nch])';
data2 = data2(:,inds);
for i = 1:size(data2,2)
    newData(:,i) = decimate(data2(:,i),downsampleby);
end
data2 = newData;
