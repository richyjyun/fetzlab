...
for i = 1:length(SL)
    if (~(strcmp(SL(i).Condition, 'Control') || strcmp(SL(i).Condition, 'Train') ))
        continue;
    end
    ...
    trig = [Trials(round(length(Trials)/3)),Trials(round(2*length(Trials)/3))];
    
    %% Left trials
    Trials = SL(i).lefttrials; RT = SL(i).rts_l; Epochs = []; Stim = [];
    bounds = [find(Trials(:,2)<trig(1),1,'last'),find(Trials(:,1)>trig(end),1)];
