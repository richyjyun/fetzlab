...
for i = 1:length(SLNeuro)
    ...
    if(strcmp(epoch,'post'))
        if(normalized)
            ...
            Beta.L(end+1,:) = (max(a,[],3)-max(b,[],3))./(mean(a(1,:,1:base),3)-mean(b(1,:,1:base),3));
            a = SLNeuro(i).Rbeta(end,:,inds); b = SLNeuro(i).Rbeta(1,:,inds);
            Beta.R(end+1,:) = (max(a,[],3)-max(b,[],3))./(mean(a(1,:,1:base),3)-mean(b(1,:,1:base),3));
            ...
            Beta.L(end+1,:) = max(SLNeuro(i).Lbeta(end,:,inds),[],3)-max(SLNeuro(i).Lbeta(1,:,inds),[],3);
            Beta.R(end+1,:) = max(SLNeuro(i).Rbeta(end,:,inds),[],3)-max(SLNeuro(i).Rbeta(1,:,inds),[],3);
    ...
    elseif(strcmp(epoch,'cond'))
        Beta.L(end+1,:) = max(SLNeuro(i).Lbeta(2,:,inds),[],3)-max(SLNeuro(i).Lbeta(1,:,inds),[],3);
        Beta.R(end+1,:) = max(SLNeuro(i).Rbeta(2,:,inds),[],3)-max(SLNeuro(i).Rbeta(1,:,inds),[],3);
    ...
    if(strcmp(SL(SLInd).Animal,'Igor'))
        ...
        for s = 1:length(StimSites)
            ...
            if(~isempty(site))
                Beta.L(end,site) = nan; Beta.R(end,site) = nan;
                Gamma.L(end,site) = nan; Gamma.R(end,site) = nan;
            end
        end
    end
    ...
    StimDelay(end+1) = SL(SLInd).NormDelay;
...
for chn = 1:length(SLNeuro(1).chID)
    ...
    scatter(RT.L,freq.L(:,chn),'k.'); ylim([-2,1]);  %ylim([-0.3,0.3])
...
%     bar(mid,binned,1); xticks(sort([edges(1),0,edges(end)])); xlim([edges(1),edges(end)]);    
    ...
    scatter(RT.R,freq.R(:,chn),'r.'); ylim([-2,1]);  %ylim([-0.3,0.3])
