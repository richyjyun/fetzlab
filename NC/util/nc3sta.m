function [sta, x, n, names] = nc3sta(trig_times, targ_chan_ids, pre_trig_offset_ms, post_trig_offset_ms, bins_per_second, filter_band, file_name)
% [STAs x n names] = nc3sta(trig_times, targ_chan_ids, pre_trig_offset_ms, post_trig_offset_ms, bins_per_second, filter_band, file_name)
% Calculates an event triggered average between the trigger times and the
% target channels.
%
% trig_times -- event times given in seconds from the beginning of the file.
% targ_chan_id -- list of channel ID numbers.  Use [1..16] for first 16
% channels, ([0..31] for pre v320 files)
...
%
% Example:
% >> [STAs, x, n] = nc3sta(trigs, [1:16], -50, 100, 10000, [500 2500]);
% >> figure;
...
for ichan = 1:nchans
    if rates(ichan) > 0
        ...
        if (idcheck >= 32) && (idcheck <= 34)
            % Handle Aux channels as unsigned values
            cname = [fpath fprefix '_Chan' num2str(id, '%.02d') '.u16'];
            adu2units = 2.4 / 65536; % Aux channels are in Volts (2.4 Volts at max unsigned 16-bit integer)
            ctype = 'uint16';
        end
        ...
        if (fid ~= -1) && ~isempty(start_trig)          
            ...
            for itrig = start_trig:ntrigs
                ...
                [dat, count] = fread(fid, [sweep_samples(ichan), 1], ctype);
            ...
            sta(:, ichan) = ave * adu2units / nsweeps;  % convert to uV
            fclose(fid);
        end
        n(ichan) = nsweeps;
    end
